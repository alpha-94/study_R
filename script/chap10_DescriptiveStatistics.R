...
# 무지개 색으로 7명 중 6명이 빨 주 노 초 파 남 으로 뽑은 경우 나머지의 색은 ? -> 보라색 :: 명백히 알 수 있음 
# 즉 표본으로 자유도를 N-1 할 경우 나머지 한개의 명백한 값을 주어지기 위해 -1 을 취함 
...
# 데이터 특성 보기 
dim(data) # [1] 300   8 - 차원 보기 
length(data) # 8 컬럼 개수
length(data$survey) # 300 레코드 개수
...
table(data$gender) # 각 성별 빈도수 - outlier(이상치) -> 0,5
#0   1   2   5 
#2 173 124   1
...
# 학력수준 변수의 빈도수 시각화
x1 <- table(data$level) # 각 학력수준에 빈도수 자장
barplot(x1)
...
# 2.3 등간척도 기술 통계량
# 만족도 변수 대상 요약 통계량 구하기
survey <- data$survey
str(survey)

summary(survey) # 만족도 (5점 척도) 인 경우 의미 있음 - > 2.605(Mean)
...
summary(data$cost) # 요약 통계량 - 의미있음 (mean ::  8.784, median :: 3.00)

plot(data$cost)

# 데이터 정제 (이상치 제거)
data <- subset(data, data$cost >= 2 & data$cost <= 10) # 총점 기준 
data 

plot(data$cost)
x <- data$cost
mean(x) # 8.784 -> 5.354032
...
var(x) # 분산, [1] 1.296826 ## 퍼짐의 정도 - 평균을 기준 // 제곱의 방법으로 분산 표기 
...
# 왜도 - 평균을 중심으로 기울어진 정도.
skewness(cost) # [1] -0.297234 :: 약간 오른쪽으로 틀어짐 


# 첨도 - 표준 정규 분포와 비교하여 얼마나 뽀족한가 측정 지표 
kurtosis(cost) # [1] 2.674163 :: 위로 솟음 
