...
# 2) 방향성을 갖는 단측 가설 검정 
binom.test(c(136, 14), p=0.8, alternative = "greater", conf.level = 0.95) # p-value = 0.0003179 긍정의 효과
...
t.test(x1, mu=5.2) # p-value = 0.0001417 mu :: 기준치값 -> 연구가설 채택 
t.test(x1, mu=5.2, alternative = "two.side", conf.level = 0.95)


# 2. 방향성을 갖는 단측가설 검정 
t.test(x1, mu=5.2, alternative = "greater", conf.level = 0.95) # p-value = 7.083e-05
...
# p-value = 0.0001711 < 0.05(PT < Coding) :: 채택
