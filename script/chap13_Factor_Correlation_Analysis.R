...
###########################################
# Chapter13-1. 요인분석(Factor Analysis)
###########################################
...
#정수로 기입했으나, R에서는 디폴트로 실수로 저장. 기본적으로 numeric
name <-1:10 # 각 과목 문항 이름

subject <- data.frame(s1, s2, s3, s4, s5, s6)
View(subject)
str(subject) #'data.frame':	10 obs. of  6 variables:
summary(subject)

# [실습] 변수의 주요 성분분석 
pc <- prcomp(subject) #주성분 분석은 'prcomp'라는 함수를 이용 데이터를 표준화 시키면서 주성분 분석
View(pc)
summary(pc)
#                        PC1    PC2     PC3     PC4     PC5     PC6
#Standard deviation     2.389 1.5532 0.87727 0.56907 0.19315 0.12434 표준편차 
#Proportion of Variance 0.616 0.2603 0.08305 0.03495 0.00403 0.00167 분산의 비율
#Cumulative Proportion  0.616 0.8763 0.95936 0.99431 0.99833 1.00000 누적되는 비율값(0.616+0.2603+...)
#하나의 그룹으로 묶으면 약 62%, 두 그룹으로 묶으면 약 87% 차지. %를 보면서 몇개 그룹으로 묶을지 확인
#3개 그룹으로 묶을 때 약 96% 차지함. 4그룹과 차이가 나지 않음. 해서 3개 그룹핑을 하면 대부분이 유지 될 수 있다고 판단. 
plot(pc) #Proportion of Variance 를 %해서 보여 줌


# 고유값으로 요인 수 분석 
en <- eigen(cor(subject)) # eigen : $values : 고유값, $vectors : 고유벡터 / cor-> Correlation Analysis 상관관계 분석, 관련성
names(en) # "values"  "vectors"
#en$values
#en$vectors
en$values # $values : 고유값(스칼라) 보기 # 상관계수의 값 : 3.44393944 1.88761725 0.43123968 0.19932073 0.02624961 0.01163331 : 
en$vectors
#           [,1]         [,2]        [,3]       [,4]        [,5]        [,6]
...
result
# Uniquenesses:0.5 이하면 유효 ! 

"
Loadings:
   Factor1 Factor2 Factor3
s1 -0.379           0.923v 
s2 -0.710   0.140   0.649v 
s3  0.236   0.931v   0.166 
s4  0.120   0.983v  -0.118 
s5  0.771v   0.297  -0.278 
s6  0.900v   0.301  -0.307 
...
# (3) 다양한 방법으로 요인적재량 보기
print(result, digits = 2, cutoff=0.5) #문자열과 함께 출력 하고 싶으면 :: cat
...
# [실습] 요인분석에 사용될 데이터 셋 가져오기--> Q4 가 잘못된 요인 --> 이 잘못된 요인을 제거 
...
######################################################
## Chapter13-2. 상관관계 분석(Correlation Analysis)
######################################################
...
cor(result$제품_친밀도, result$제품_적절성) # 0.4992086 -> 다소 높은 양의 상관관계  0.4~0.7 기준
cor(result$제품_친밀도, result$제품_만족도) # 0.467145 -> 다소 높은 양의 상관관계

# [실습] 전체 변수 간 상관계수 보기
cor(result, method="pearson") # 디폴트 :: pearson
